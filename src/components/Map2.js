...
function Map() {
  ...
  const displayStates =(data) => {
    ...
    return <path fill= {colorTheme}  key={key} className={el.props.className} d={el.props.d}><title>{el.props.children.props.children}</title></path>
    })
  return states
  }
  ...
  function changeColor(e){
    // seperate array methods will run depending on which selection was selected
    if(e.target.value ==='firstStates') {
      // loop through rangeOne array and compare elements to classNames in the statesMap
      // if they match change the color
      statesMap.forEach(el => {
        if(el.props.className === 'al') {
        el.props.fill = setColorTheme('#0D7EF9')
        }
      })  
  } else {
    //continue same procedure for 250-500/500-1000/1000+s
  }
  }
